...
public class BoardDBBean {
	...
	public int getArticleCount() throws Exception {
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement("select count(*) from board_00");
			rs = pstmt.executeQuery();
	...
	public int updateArticle(BoardDataBean article) throws Exception {
		...
		if(rs.next()) {
			...
			if(dbpasswd.equals(article.getPasswd())) {
				sql="update board_00 set writer=?,email=?,subject=?,passwd=?";
		...
		try {
			...
			if(rs.next()) {
				...
				if(dbpasswd.equals(passwd)) {
					pstmt = conn.prepareStatement(
							"delete from board_00 where num=?");
					pstmt.setInt(1, num);
					pstmt.executeUpdate();
				x=1;	
				} else
	...
	public int getArticleCount(int n, String searchKeyword) throws Exception {
		...
		try {
			conn=getConnection();
			pstmt = conn.prepareStatement("select count(*) from board_00 where "+column_name[n]+" like '%"+searchKeyword+"%'");
			rs = pstmt.executeQuery();
